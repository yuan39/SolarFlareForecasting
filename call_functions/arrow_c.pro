; $Id: arrow.pro,v 1.4 1998/01/15 18:40:33 scottm Exp $

;
; Copyright (c) 1993-1998, Research Systems, Inc.  All rights reserved.
;	Unauthorized reproduction prohibited.
;
PRO ARROW_C, x0, y0, x1, y1, HSIZE = hsize, COLOR = color, HTHICK = hthick, $
	THICK = thick, DATA = data, DEVICE = device, NORMALIZED = norm, $
	SOLID = solid
;+
; NAME:				ARROW_C
; PURPOSE:	Draw a vector(s) with an arrow head (modified version of ARROW)
; CATEGORY:	Graphics
; CALLING SEQUENCE:
;	ARROW_C, x0, y0, x1, y1
; INPUTS:
...
;sint = .500
mcost = -.9397		;We use 20 degrees for head angle
sint = .3420
...
for i = 0, n_elements(x0)-1 do begin		;Each vector
	if keyword_set(data) then $		;Convert?
	    p = convert_coord([x0[i],x1[i]],[y0[i],y1[i]], /data, /to_dat) $
	else if keyword_set(norm) then $
	    p = convert_coord([x0[i],x1[i]],[y0[i],y1[i]], /norm, /to_dat) $
	else p = convert_coord([[x0[i], y0[i]],[x1[i], y1[i]]], /dev, /to_dat)
	...
	if keyword_set(solid) then begin	;Use polyfill?
	  ...
	  oplot, [xp0, xp1+b*dx], [yp0, yp1+b*dy], $
		COLOR = color, THICK = thick
	  polyfill, [xxp0, xxp1, xp1, xxp0], [yyp0, yyp1, yp1, yyp0], $
		/DEVICE, COLOR = color
	endif else begin
	  oplot, [xp0, xp1], [yp0, yp1],  COLOR = color, THICK = thick
	  oplot, [xxp0,xp1,xxp1],[yyp0,yp1,yyp1],  COLOR = color, $
			THICK = hthick
	endelse
	ENDFOR
end
