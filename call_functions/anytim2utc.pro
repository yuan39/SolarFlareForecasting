      ...
      'String':  begin
		    for i=0L,np-1 do begin
                        ...
                        if ndash eq 1 and nslash eq 1 then begin
                           get_utc,dutc
                           date = anytim2cal(dutc,form=9,/date,_extra=_extra)
			   ...
			   if message eq '' then utc(i) = test
                    	   ...
                    	   test = str2utc(dt(i), errmsg=errmsg,_extra=_extra)
		    	   if n_elements(errmsg) ne 0 then message=errmsg
			   if message eq '' then utc(i) = test
                        endelse
		    endfor
...
;
		    ...
		    sz = size(dt)
		    if (sz(0) gt 0) and (sz(1) eq 7) then begin
			...
			if sz(0) gt 1 then begin
			    ...
			    utc = reform(utc,sz(2:sz(0)),/overwrite)
			endif
			...
			if keyword_set(external) then return, utc else $
